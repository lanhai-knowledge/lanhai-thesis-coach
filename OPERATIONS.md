...
- **From = To = `b0915905438@gmail.com`**：自己寄給自己 deliverability 最高，不會被 Gmail 標 spam。Reply-To 才是預約者 email，Gmail 按「回覆」就直接回到客戶信箱。
...
1. 用 `b0915905438@gmail.com` 登入 Google 帳戶
...
4. 在文字框貼上以下內容（把 `SMTP_PASS` 換成 Step 1 拿到的 16 碼）：
   ...
   SMTP_PORT = 465
   SMTP_USER = "b0915905438@gmail.com"
   SMTP_PASS = "abcd efgh ijkl mnop"
   BOOKING_TO = "b0915905438@gmail.com"
   ```
...
3. 按「送出預約」
4. 預期看到綠字「✅ 預約資訊已寄出」
5. 30 秒內到 `b0915905438@gmail.com` 收件匣確認：
   - 主旨格式：`【嵐海預約】測試-XXXX｜尚未決定，希望初談後再評估`
   - 內文是純文字、欄位齊全
   - 在 Gmail 按「回覆」→ To 自動帶到你 Step 2 填的另一個信箱（不是 b0915905438）
